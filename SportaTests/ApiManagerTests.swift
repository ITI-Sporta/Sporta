...
class ApiManagerTests: XCTestCase {
    ...
    func test_fetch_past_fixtures_returns_data() {
        ...
        apiManager.fetchPastFixtures(for: .football, leagueId: 4) { result in
            switch result {
    ...
    func test_fetch_upcoming_fixtures_returns_data() {
        ...
        apiManager.fetchUpcomingFixtures(for: .football, leagueId: 4) { result in
            switch result {
            case .success(let fixtures):
                XCTAssertFalse(fixtures.isEmpty)
                exp.fulfill()
    ...
    func test_fetch_teams_returns_data() {
        ...
        apiManager.fetchTeams(for: .football, leagueId: 4) { result in
            switch result {
    ...
    func test_fetch_team_fixtures_returns_data() {
        ...
        apiManager.fetchTeamDetailsFixtures(for: .football, teamId: 4) { result in
            switch result {
    ...
    func test_fetch_team_details_returns_data() {
        ...
        apiManager.fetchTeamDetails(for: .football, teamId: 4) { result in
            switch result {
